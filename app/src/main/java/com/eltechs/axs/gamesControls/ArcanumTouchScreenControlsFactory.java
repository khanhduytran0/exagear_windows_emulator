...
public class ArcanumTouchScreenControlsFactory implements TouchScreenControlsFactory {
    ...
    private void fillTouchScreenControls(TouchScreenControls touchScreenControls, View view, ViewOfXServer viewOfXServer) {
        ...
        if (view.getWidth() > displayMetrics.widthPixels / 2) {
            ...
            this.gestureContext = GestureMachineConfigurerArcanum.createGestureContext(viewOfXServer, touchArea, touchEventMultiplexor, displayMetrics.densityDpi, new Runnable() {
                public void run() {
